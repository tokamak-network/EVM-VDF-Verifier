...
const deployCRRRNGCoordinator: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
    ...
    log("----------------------------------------------------")
    const crrRngCoordinator = await deploy("CRRNGCoordinatorPoFForTitan", {
        from: deployer,
        log: true,
        args: [
    ...
    // deploy result
    log("CRRNGCoordinatorPoFForTitan deployed at:", crrRngCoordinator.address)
...
deployCRRRNGCoordinator.tags = ["all", "CRRNGCoordinator", "anvil", "titan", "titanCoordinator"]
