...
!developmentChains.includes(network.name)
  ...
  /____/_/__/___/_/ /_//_/   /_/|_/_/|_| /_/   ___/___/  \n",
          ...
          describe(`put h and bStar value on chain`, () => {
              before(async () => {
                  console.log("putting h and bStar on chain...")
                  tx = await commitRecover.connect(deployer).setHAndBStar(h)
                  txReceipt = (await tx.wait(1)) as ContractTransactionReceipt
              ...
              it("should emit SetHAndBStar event correctly", async () => {
                  const bStar = ethers.solidityPackedKeccak256(["string"], [commitsStringTest])
                  await expect(txReceipt)
                      .to.emit(commitRecover, "SetHAndBStar")
                      .withArgs(deployer.address, h, bStar, setHTimestamp)
              })
              ...
              it("should set isHset to true", async () => {
                  const isHset = await commitRecover.isHAndBStarSet()
              ...
              describe(`--reveal_${memberToReveal}--`, () => {
                  ...
                  describe(`--reveal result--`, () => {
                      ...
                      it("bStar should be set keccak256(abi.encodePacked(commitsSring)) correctly", async () => {
                          /// check bStar
                          const bStarOnChain = (await commitRecover.omegaAtRound(round)).bStar
                          bStar = ethers.solidityPackedKeccak256(["string"], [commitsStringTest])
                      ...
                      it("if count == 0, then isOmegaCompleted = true", async () => {
                          ...
                          isOmegaCompleted = Boolean(
                              (await commitRecover.omegaAtRound(round)).isCompleted,
                          )
              ...
              describe("recover...", () => {
                  ...
                  describe("recover result...", () => {
                      ...
                      it("omegaAtRound[round] should be the same as omega", async () => {
                          const omegaAtRoundOmega = Number(
                              (await commitRecover.omegaAtRound(round)).omega,
                          )
                          assert.equal(omegaAtRoundOmega, omega)
                      })
                  })
              })
          }
          let startedTimestamp: number
          let args: CommitRecover.StartParamsStruct
          describe("start again..", () => {
              before(async () => {
                  ...
                  txReceipt = (await tx.wait(1)) as ContractTransactionReceipt
                  const startedTxBlock = await ethers.provider.getBlock(
                      txReceipt?.blockNumber as number,
                  )
                  startedTimestamp = startedTxBlock?.timestamp as number
              })
              it("should emit Start event correctly", async () => {
                  round++
                  await expect(txReceipt)
                      .to.emit(commitRecoverContract, "Start")
                      .withArgs(
                          deployer.address,
                          startedTimestamp,
                          args.commitDuration,
              ...
              it("isOmegaCompleted should be false", async () => {
                  /// check isOmegaCompleted
                  isOmegaCompleted = Boolean((await commitRecover.omegaAtRound(round)).isCompleted)
                  assert.equal(isOmegaCompleted, false)
              })
              ...
              for (i = 0; i < revealNum; i++) {
                  ...
                  describe(`--reveal_${memberToReveal}--`, () => {
                      before(async () => {
                          ...
                          commitRecover = commitRecoverContract.connect(
                              accounts[memberToReveal + 1],
                          )
                          tx = await commitRecover.reveal(a[memberToReveal])
                          revealCount++
                          txReceipt = (await tx.wait(1)) as ContractTransactionReceipt
                      ...
                      describe(`--reveal result--`, () => {
                          it("should emit RevealA event correctly", async () => {
                              ...
                              const commitDuration = Number(await commitRecover.commitDuration())
                              console.log(
                                  (revealedTimestamp as number) -
                                      (commitStartTime + commitDuration),
                                  " seconds passed since reveal started",
                              )
                          })
                          it("isHSet should be still true", async () => {
                          ...
                          it("bStar should be set keccak256(abi.encodePacked(commitsSring)) correctly", async () => {
                              /// check bStar
                              const bStarOnChain = (await commitRecover.omegaAtRound(round)).bStar
                              bStar = ethers.solidityPackedKeccak256(
                                  ["string"],
                                  [commitsStringTest],
                              )
                              assert.equal(bStar, bStarOnChain)
