...
describe("Optimized Pietrzak Verification3", () => {
        ...
        for (let k: number = 0; k < deltas.length; k++) {
            ...
            for (let i: number = 0; i < lambdas.length; i++) {
                for (let j: number = 0; j < Ts.length; j++) {
                    ...
                    const calldataBigNumber =
                        verifyRecursiveHalvingProofDeltaBigNumber.interface.encodeFunctionData(
                    ...
                    const calldataExternal =
                        verifyRecursiveHalvingProofExternal.interface.encodeFunctionData(
                            "verifyRecursiveHalvingProofExternal",
                            [
                                recoveryProofs,
                                x,
                                ...
                                toBeHex(
                                    2n ** BigInt(delta),
                                    getLength(dataLength(toBeHex(2n ** BigInt(delta)))),
                                ),
                                2n ** BigInt(delta),
                                shift128TestCase.T,
                            ],
                        )
                    const gasUsedExternal =
                        await verifyRecursiveHalvingProofExternal.verifyRecursiveHalvingProofExternal.estimateGas(
                            recoveryProofs,
                            x,
                            y,
                            shift128TestCase.n,
                    ...
                    const trueOrFalse2 =
                        await verifyRecursiveHalvingProofDeltaBigNumber.verifyRecursiveHalvingProofExternal(
                            recoveryProofs,
                            x,
                            y,
                            shift128TestCase.n,
                    ...
                    data.push([
                        ...
                        lambdas[i],
                        Ts[j],
                        gasUsedBigNumber.toString(),
                        gasUsedExternal.toString(),
                        (gasUsedExternal - gasUsedBigNumber).toString(),
                        (calldataBigNumber.length / 2).toString(),
                        (calldataExternal.length / 2).toString(),
                        (calldataBigNumber.length - calldataExternal.length).toString(),
                    ])
                }
            }
        }
        console.log(
            "[delta, lambda, T, gasUsed2^2^delta, gasUsed2^delta, 2^delta - 2^2^delta gasUsed, calldataInBytes2^2^delta, calldataInBytes2^delta, 2^2^delta - 2^delta calldata]",
        )
        console.log(data)
        // delta8JsonData["2048"] = data
        // fs.writeFileSync(DELTA_8_FILENAME, JSON.stringify(delta8JsonData, null, 2))
    })
    it("2^2^delta or 2^delta 2048", async () => {
        const verifyRecursiveHalvingProofDeltaBigNumber = await loadFixture(
            deployVerifyRecursiveHalvingProofDeltaBigNumberContract,
        )
        const verifyRecursiveHalvingProofExternal = await loadFixture(
    ...
    it("delta BigNumber gasUsed 2048", async () => {
        const verifyRecursiveHalvingProofDeltaBigNumber = await loadFixture(
            deployVerifyRecursiveHalvingProofDeltaBigNumberContract,
        )
        const lambdas: string[] = ["λ2048"]
        ...
        for (let k: number = 0; k < deltas.length; k++) {
            ...
            for (let i: number = 0; i < lambdas.length; i++) {
                for (let j: number = 0; j < Ts.length; j++) {
                    ...
                    assert(trueOrFalse2)
                    data.push([delta.toString(), gasUsedBigNumber.toString(), lambdas[i], Ts[j]])
                }
            }
        }
        console.log(data)
    ...
    it("delta BigNumber gasUsed 3072", async () => {
        const verifyRecursiveHalvingProofDeltaBigNumber = await loadFixture(
            deployVerifyRecursiveHalvingProofDeltaBigNumberContract,
        )
        const lambdas: string[] = ["λ3072"]
    ...
    it("delta BigNumber gasUsed 2048 T 22~24 d", async () => {
        const verifyRecursiveHalvingProofDeltaBigNumber = await loadFixture(
            deployVerifyRecursiveHalvingProofDeltaBigNumberContract,
        )
        const lambdas: string[] = ["λ2048"]
    ...
    it("delta BigNumber gasUsed 3072 T 22~24 d", async () => {
        const verifyRecursiveHalvingProofDeltaBigNumber = await loadFixture(
            deployVerifyRecursiveHalvingProofDeltaBigNumberContract,
        )
        const lambdas: string[] = ["λ3072"]
