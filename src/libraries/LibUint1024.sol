...
library LibUint1024 {
    ...
    function toUint1024(
        uint256 x
    ) internal pure returns (uint256[4] memory bn) {
        bn[3] = x;
    }
    ...
    function add(
        uint256[4] memory a,
        uint256[4] memory b
    ) internal pure returns (uint256[4] memory c) {
        uint256 carry;
        (c, carry) = _add(a, b);
        if (carry != 0) {
            revert Overflow(a, b);
    ...
    function _add(
        uint256[4] memory a,
        uint256[4] memory b
    ) internal pure returns (uint256[4] memory c, uint256 carry) {
    ...
    function mulMod(
        ...
        assembly ("memory-safe") {
            ...
            if iszero(
                staticcall(gas(), 0x04, modulus, 0x80, add(p, 0x180), 0x80)
            ) {
                revert(0, 0)
            }
    ...
    function addMod(
        ...
        uint256[4] memory b,
        uint256[4] memory modulus
    ) internal pure returns (uint256[4] memory result) {
        uint256 carry;
    ...
    function expMod(
        ...
        assembly ("memory-safe") {
            ...
            // Use Identity (0x04) precompile to memcpy the modulus
            if iszero(
                staticcall(gas(), 0x04, modulus, 0x80, add(p, 0x100), 0x80)
            ) {
                revert(0, 0)
            }
            // Call 0x05 (EXPMOD) precompile
    ...
    function expMod(
        ...
        assembly ("memory-safe") {
            ...
            if iszero(
                staticcall(gas(), 0x04, exponent, 0x80, add(p, 0xe0), 0x80)
            ) {
                revert(0, 0)
            }
            // Use Identity (0x04) precompile to memcpy the modulus
            if iszero(
                staticcall(gas(), 0x04, modulus, 0x80, add(p, 0x160), 0x80)
            ) {
                revert(0, 0)
            }
            // Call 0x05 (EXPMOD) precompile
    ...
    function normalize(
        uint256[4] memory x,
        uint256[4] memory modulus
    ) internal pure returns (uint256[4] memory normalized) {
