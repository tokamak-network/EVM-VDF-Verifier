...
/**
 * @title VDF that performs commit-recover using VDF verification
...
contract VDFPoF is ReentrancyGuardTransient, GetL1Fee {
    ...
    mapping(uint256 round => uint32 callbackGasLimit)
    ...
    mapping(address operator => uint256 disputeEndTime)
        internal s_disputeEndTimeForOperator;
    /// @dev The mapping of all the incentive for the operator
    mapping(address operator => uint256 depositAmount)
        internal s_depositedAmount;
    ...
    mapping(uint256 round => uint256 disputeEndTime)
    ...
    error RNGCoordinator_NotOperator();
    ...
    modifier onlyOperator() {
        if (!s_isOperators[msg.sender]) revert RNGCoordinator_NotOperator();
        _;
    }
    ...
    function commit(
        ...
        if (s_operatorStatusAtRound[round][msg.sender].committed) {
            ...
            if (
                block.timestamp <
                s_operatorStatusAtRound[round][msg.sender].maxCommitTimestamp
            ) revert AlreadyCommitted();
            if (BigNumbers.eq(s_commitValues[round][commitIndex], c))
                revert CommittedSameValue();
            unchecked {
                --s_ignoredCounts[round];
            }
            s_commitValues[round][commitIndex] = c;
            s_operatorStatusAtRound[round][msg.sender]
                .maxCommitTimestamp = commitEndTime;
    ...
    function disputeRecover(
        ...
        for (uint256 i = 2; i < _commitCounts; i = _unchecked_inc(i)) {
            _commitString = bytes.concat(
                _commitString,
                s_commitValues[round][i].val
            );
        }
    ...
    function disputeLeadershipAtRound(uint256 round) external onlyOperator {
        ...
        bytes32 _leaderHash = keccak256(
            abi.encodePacked(_omega, previousLeader)
        );
        ...
        emit FulfillRandomness(
            round,
            s_fulfillStatus[round].succeeded,
            msg.sender
        );
    }
    ...
    function getCommitValuesAtRound(
        uint256 round
    ...
    function getFulfillStatusAtRound(
        uint256 round
    ) external view returns (FulfillStatus memory) {
        return s_fulfillStatus[round];
    }

    function _getDisputeLeadershipTxGasFee() private view returns (uint256) {
        return (_getDisputeLeadershipTxL1GasFee() +
            (L2_DISPUTELEADERSHIP_TX_GAS * tx.gasprice));
    }

    function _getDisputeRecoverTxGasFee() internal view returns (uint256) {
        return
            ((_getDisputeRecoverTxL1GasFee() +
                (L2_DISPUTERECOVER_TX_GAS * tx.gasprice)) *
    ...
    function _verifyRecursiveHalvingProof(
        ...
        if (
            !BigNumbers.eq(
                y,
                BigNumbers.modexp(x, BigNumber(EXPDELTA, EXPDELTABITLEN), n)
            )
