...
task("commitRecoverFulfillAnvil", "Operator commits For Titan")
    .addParam("round", "The round")
    .setAction(async ({ round }, { deployments, ethers, getNamedAccounts }) => {
        ...
        for (let i: number = 0; i < 3; i++) {
            const commitCount = (await crrngCoordinatorContract.getValuesAtRound(round))
                .commitCounts
        ...
        const disputePeriod = 180
        await time.increase(BigInt(disputePeriod) + 1n)
        console.log("time increased by 181 seconds")
        tx = await crrngCoordinatorContract.fulfillRandomness(round)
        await tx.wait()
