...
contract RandomDayForTitan is RNGConsumerBase, Ownable {
    ...
    mapping(address requester => RequesterInfos requesterInfos) private s_requesters;
    ...
    uint256 public constant EVENTPERIOD = 86400;
    ...
    function fulfillRandomWords(uint256 requestId, uint256 hashedOmegaVal) internal override {
        ...
        if (_avgNum != _newAvgNum) {
            ...
            s_requesterIndex[requester] = s_tickRequesters[_newAvgNum].length - 1;
        }
    }
    ...
    function finalizeRankingandSendPrize() external onlyOwner {
        if (!started) revert EventNotStarted();
        if (block.timestamp < eventEndTime) revert EventStillRunning();
        (uint256[4] memory rticks, uint256[4] memory counts) = _getThreeClosestToSevenHundred();
    ...
    function getTickRequesters(uint256 tick) external view returns (address[] memory) {
