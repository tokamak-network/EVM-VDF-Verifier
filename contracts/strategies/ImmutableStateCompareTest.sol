...
contract ImmutableStateCompareTest is ICommitRevealRecoverRNG {
    ...
    function recoverImmutable(uint256 _round, VDFClaim[] calldata proofs) external {
        //BigNumber memory _n = setUpValuesAtRound[_round].n;
    ...
    function recoverImmutable2(uint256 _round, VDFClaim[] calldata proofs) external {
        //BigNumber memory _n = setUpValuesAtRound[_round].n;
        BigNumber memory _n = BigNumber(NVAL, NBITLEN);
        //BigNumber memory _n = BigNumbers.n();
        uint256 _proofsLastIndex = proofs.length - ONE;
        checkStage(_round);
        uint256 _numOfParticipants = valuesAtRound[_round].numOfParticipants;
        if (valuesAtRound[_round].stage == Stages.Commit) revert FunctionInvalidAtThisStage();
    ...
    function setUp(
        ...
        verifyRecursiveHalvingProof(_proofs, _n, _proofsLastIndex, _T);
    ...
    function verifyRecursiveHalvingProofExternalForTestInternalGas(
        ...
        for (; i < _proofsLastIndex; i = unchecked_inc(i)) {
            ...
            BigNumber memory _xPrime = _proofList[i].x.modexp(_r, _n);
            if (!_xPrime.modmul(_proofList[i].v, _n).eq(_proofList[unchecked_inc(i)].x))
