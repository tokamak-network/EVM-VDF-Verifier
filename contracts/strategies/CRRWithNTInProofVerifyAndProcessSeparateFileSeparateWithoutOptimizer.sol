...
import {BigNumbersWithoutOptimizer} from "./libraries/BigNumbersWithoutOptimizer.sol";
import "./interfaces/ICRRWithNTInProofVerifyAndProcessSeparateFileSeparateWithoutOptimizer.sol";
import "./libraries/Pietrzak_VDFWithoutOptimizer.sol";

contract CRRWithNTInProofVerifyAndProcessSeparateFileSeparateWithoutOptimizer is
    ICRRWithNTInProofVerifyAndProcessSeparateFileSeparateWithoutOptimizer
{
    ...
    function calculateOmega(uint256 _round) external override returns (BigNumber memory) {
        ...
        for (uint256 i; i < _numOfParticipants; i = unchecked_inc(i)) {
            BigNumber memory _temp = bytes
                .concat(commitRevealValues[_round][i].c.val, _bStar)
                .modHash(_n);
    ...
    function setUp(
        ...
        _proofs.verifyRecursiveHalvingProof(_proofs[ZERO].n, _proofsSize);
        ...
        setUpValuesAtRound[_round].proofsSize = _proofsSize;
    ...
    function checkStage(uint256 _round) private {
        ...
        if (
            ...
            if (valuesAtRound[_round].count != ZERO) {
                ...
                valuesAtRound[_round].bStar = valuesAtRound[_round]
                    .commitsString
                    .modHash(setUpValuesAtRound[_round].n)
                    .val;
            } else {
                valuesAtRound[_round].stage = Stages.Finished;
            }
    ...
    //     if (i != _proofsSize || _proofList[i].T != ONE) revert TOneNotAtLast();
    // }
