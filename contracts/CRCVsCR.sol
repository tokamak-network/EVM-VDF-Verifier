...
import {ICRCVsCR} from "./interfaces/ICRCVsCR.sol";
import {RNGConsumerBase} from "./RNGConsumerBase.sol";

contract CRCVsCR is ICRCVsCR {
    /* Type declaration */
    ...
    function initialize(BigNumber[] memory v, VDFClaim2 memory xyd) external {
        if (s_verified) revert AlreadyVerified();
        require(BigNumbers.eq(BigNumber(GVAL, GBITLEN), xyd.x));
        require(BigNumbers.eq(BigNumber(HVAL, HBITLEN), xyd.y));
        _verifyRecursiveHalvingProof(
            v,
            xyd.x,
            xyd.y,
            BigNumber(NVAL, NBITLEN),
            xyd.bigNumTwoPowerOfDelta,
            xyd.delta
        );
        s_verified = true;
    }
    ...
    function calculateOmega(
        ...
        for (uint256 i; i < _numOfPariticipants; i = _unchecked_inc(i)) {
            ...
            _temp = BigNumbers.init(
                BigNumbers._modexp(
                    BigNumbers._modexp(HVAL, _temp.val, NVAL),
                    s_commitRevealValues[round][i].a.val,
                    NVAL
                )
            );
        ...
        nonReentrant
    {
        ...
        if (s_valuesAtRound[round].isCompleted) revert OmegaAlreadyCompleted();
        BigNumber memory _n = BigNumber(NVAL, NBITLEN);
        _verifyRecursiveHalvingProof(v, xyd.x, xyd.y, _n, xyd.bigNumTwoPowerOfDelta, xyd.delta);
        bytes memory _bStar = s_valuesAtRound[round].bStar;
        ...
        if (!BigNumbers.eq(_recov, xyd.x)) revert RecovNotMatchX();
        // effect
        s_valuesAtRound[round].isCompleted = true;
        s_valuesAtRound[round].omega = xyd.y;
        s_valuesAtRound[round].stage = Stages.Finished;
        //emit Recovered(round, _recov.val, proofs[BigNumbers.UINTZERO].y.val, true);
    }
